...
export default function MovieDetailsScreen({ route, navigation }) {
  ...
  const [currentCalendarMonth, setCurrentCalendarMonth] = useState(currentDate.getMonth() + 1);
  ...
  const renderCalendarModal = () => {
    ...
    return (
      <Modal animationType="slide" transparent={true} visible={showCalendar} onRequestClose={handleCloseCalendar}>
  ...
  const renderShareModal = () => {
    const shareLink = `https://www.cgv.vn/${movie?.MovieTitle?.toLowerCase().replace(/\s+/g, '-')}`;
    const shareText = `Xem phim ${movie?.MovieTitle} tại đây!`;

    const sharePlatforms = [
      { name: 'Messenger', image: 'https://i.imgur.com/5z5z5z5.png', shareUrl: `fb-messenger://share?link=${encodeURIComponent(shareLink)}`, fallbackUrl: `https://www.messenger.com/t/?link=${encodeURIComponent(shareLink)}` },
      { name: 'Facebook', image: 'https://i.imgur.com/6y6y6y6.png', shareUrl: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareLink)}`, fallbackUrl: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareLink)}` },
    ];

    const handleSharePress = (platform) => {
      Linking.openURL(platform.shareUrl).catch((err) => {
        console.error("Không thể mở URL:", err);
        if (platform.fallbackUrl) Linking.openURL(platform.fallbackUrl).catch((err) => console.error("Không thể mở URL dự phòng:", err));
      });
    };

    return (
      <Modal animationType="slide" transparent={true} visible={showShare} onRequestClose={() => setShowShare(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>Chia sẻ</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.shareScrollContainer}>
              <View style={styles.shareGrid}>
                {sharePlatforms.map((platform) => (
                  <TouchableOpacity key={platform.name} style={styles.sharePlatform} onPress={() => handleSharePress(platform)}>
                    <Image source={{ uri: platform.image }} style={styles.platformIcon} />
                    <Text>{platform.name}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>
            <TouchableOpacity style={styles.closeButton} onPress={() => setShowShare(false)}>
              <Text style={styles.closeButtonText}>Đóng</Text>
            </TouchableOpacity>
          </View>
        </View>
  ...
  if (loading) return <View style={styles.loadingContainer}><Text>Đang tải thông tin phim...</Text></View>;
      ...
      <View style={styles.contentContainer}>
        <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContentContainer}>
            ...
            <View style={styles.movieDetails}>
              ...
              <View style={styles.socialContainer}>
                <TouchableOpacity style={styles.likeButton} onPress={handleLikePress}>
                  <Ionicons name="heart" size={20} color={isLiked ? 'red' : 'gray'} />
                  <Text>{likes}</Text>
                </TouchableOpacity>
          ...
          <View style={styles.movieInfoSection}>
            ...
            <Text style={styles.infoValue}>{isExpanded ? fullDescription : shortDescription}</Text>
            ...
            <View style={styles.movieInfoDetail}>
              ...
              <Text style={styles.infoValue}>{movie.MovieActor || 'Chưa có thông tin'}</Text>
            </View>
            <View style={styles.movieInfoDetail}>
            ...
            <View style={styles.movieInfoDetail}>
              ...
              <Text style={styles.infoValue}>{new Date(movie.MovieReleaseDate).toLocaleDateString('vi-VN')}</Text>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  playButtonOverlay: { position: 'absolute', top: '50%', left: '50%', transform: [{ translateX: -30 }, { translateY: -30 }], zIndex: 10 },
  posterContainer: { position: 'relative', marginTop: 10 },
  thumbnailPoster: { width: 85, height: 120, borderRadius: 10, position: 'absolute', bottom: -70, left: 15, zIndex: 20 },
  movieInfoRow: { flexDirection: 'row', padding: 15, alignItems: 'center', gap: 15 },
  movieDetails: { flex: 1, marginLeft: 90 },
  showtimeContainer: { flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', gap: 15 },
  calendarButton: { flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1, borderColor: '#ccc', padding: 5, borderRadius: 5 },
  calendarButtonText: { fontSize: 14 },
  durationContainer: { borderWidth: 1, borderColor: '#ccc', padding: 5, borderRadius: 5 },
  socialContainer: { flexDirection: 'row', justifyContent: 'flex-start', marginTop: 10, gap: 15 },
  ...
  newsHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 15, marginVertical: 20 },
  allButton: { borderWidth: 1, borderColor: '#ccc', paddingVertical: 5, paddingHorizontal: 10, borderRadius: 5 },
  ...
  bookTicketButton: { position: 'absolute', bottom: 0, borderRadius: 20, left: 0, right: 0, backgroundColor: 'red', padding: 15, alignItems: 'center' },
  bookTicketButtonText: { color: 'white', fontWeight: 'bold', fontSize: 18 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' },
  modalContainer: { width: '90%', backgroundColor: 'white', borderRadius: 10, padding: 20 },
  ...
  calendarDay: { width: '14%', aspectRatio: 1, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#eee' },
  ...
  platformIcon: { width: 60, height: 60, borderRadius: 30, backgroundColor: '#eee' },
  ...
  retryButton: { marginTop: 20, backgroundColor: '#ff4d6d', padding: 10, borderRadius: 5 },
