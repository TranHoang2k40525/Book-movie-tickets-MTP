...
const TheaterLocations = memo(
  ({
    ...
    const fetchShowtimes = useCallback(
      async (cinemaId) => {
        const cacheKey = `showtimes-${movieId}-${cinemaId}-${
          selectedDate.toISOString().split("T")[0]
        }`;
        if (cache.has(cacheKey)) {
          setShowtimes((prev) => ({
            ...prev,
            [cinemaId]: cache.get(cacheKey),
          }));
          return;
        }

        try {
          const response = await getShowtimesByCinemaAndDate(
            movieId,
            cinemaId,
            selectedDate.toISOString().split("T")[0]
          );
      ...
      (show, theater) => {
        ...
        return (
          <TouchableOpacity
            ...
            style={[
              styles.timeButton,
              show.isPassed && styles.passedTimeButton,
          ...
          >
            <Text
              style={[
                styles.timeText,
                show.isPassed && styles.passedTimeText,
      ...
      (theater) => (
        <View key={`theater-${theater.id}`} style={styles.theaterContainer}>
          <TouchableOpacity
            ...
            onPress={() => {
              setExpandedTheater(
                expandedTheater === theater.id ? null : theater.id
              );
          ...
          {expandedTheater === theater.id && (
            ...
            >
              {(showtimes[theater.id] || []).map((show) =>
                renderShowtime(show, theater)
              )}
            </ScrollView>
          )}
    ...
    return (
      <ScrollView style={styles.theatersScrollView}>
        {localCinemas.map(renderTheater)}
      </ScrollView>
    );
  }
);
...
const MovieBookingScreen = ({ navigation, route }) => {
    ...
    async (retries = 3, delay = 1000) => {
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          if (attempt === retries) {
            setErrorMovie(
              err.message || "Đã có lỗi xảy ra khi tải thông tin phim"
            );
            setLoadingMovie(false);
          } else {
  ...
  const fetchCinemasData = useCallback(
    async (retries = 3, delay = 1000) => {
      const cinemasCacheKey = `cinemas-${movieId}-${
        selectedDate.toISOString().split("T")[0]
      }`;
      ...
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          ...
          const cinemasResponse = await getCinemasByMovieAndDate(
            movieId,
            selectedDate.toISOString().split("T")[0]
          );
          ...
          if (attempt === retries) {
            setErrorCinemas(
              err.message || "Đã có lỗi xảy ra khi tải danh sách rạp"
            );
            setLoadingCinemas(false);
          } else {
            await new Promise((resolve) => setTimeout(resolve, delay));
        ...
        {loadingCinemas ? (
          ...
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>
              Không có rap nào chiếu phim này ngày hôm nay{" "}
            </Text>
          </View>
        ) : (
          <TheaterLocations
