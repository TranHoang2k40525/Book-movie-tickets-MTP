...
// Updated rows and seats per row to match the image
...
// Updated seat prices (you can adjust these as needed)
const seatPrices = {
  A: 60000,
  B: 60000,
  C: 60000,
  D: 70000, // Slightly different price for row D (brown seats)
  E: 80000,
  F: 80000,
  G: 80000,
  H: 90000, // Sweetbox seats
  I: 90000, // Sweetbox seats

};

// Updated seat types to match the image
const seatTypes = {
  booked: "#A67C52", // Brown for row D
  selected: "#0047AB",
  vip: "#FF0000", // Red for rows E, F, G
  regular: "#D3D3D3", // Gray for rows A, B, C, J
  sweetbox: "#FF00FF", // Pink for rows H, I
  gray: "#808080",
};
...
export default function SeatSelection() {
  ...
  const toggleSeat = (seat) => {
    setSelectedSeats((prev) => {
      ...
      const isSweetBox = seat.startsWith("H") || seat.startsWith("I"); // H and I are sweetbox
  ...
  const Seat = ({ row, index, isSelected, onPress, minimap = false }) => {
    ...
    // Assign seat types based on the row
    if (["A", "B", "C","D", "J"].includes(row)) seatColor = seatTypes.gray; // Gray seats
    
    else if (["E", "F", "G"].includes(row)) seatColor = seatTypes.vip; // Red seats for VIP
    else if (["H", "I"].includes(row)) seatColor = seatTypes.sweetbox; // Pink seats for sweetbox
